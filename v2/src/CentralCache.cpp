        ...
        if (!result)
        {
            ...
            {  // 确保至少有两个块才构建链表
                ...
                *reinterpret_cast<void**>(result) = nullptr;
                ...
                {
                    // 先写入元数据，最后再发布 spanAddr，避免读到半初始化条目
                    spanTrackers_[trackerIndex].numPages.store(numPages, std::memory_order_relaxed);
                    spanTrackers_[trackerIndex].blockCount.store(blockNum, std::memory_order_relaxed);
                    spanTrackers_[trackerIndex].freeCount.store(blockNum - 1, std::memory_order_relaxed); // 第一个块result已被分配出去，所以初始空闲块数为blockNum - 1
...
{
    // 遍历 spanTrackers_ 数组，注意限制上界，避免 spanCount_ 超过数组长度
    size_t publishedCount = spanCount_.load(std::memory_order_acquire);
    size_t limit = (publishedCount < spanTrackers_.size()) ? publishedCount : spanTrackers_.size();

    for (size_t i = 0; i < limit; ++i)
    {
        void* spanAddr = spanTrackers_[i].spanAddr.load(std::memory_order_acquire);
        if (!spanAddr) continue;
